...
contract GlobalPauseController is ProxyOwnableUpgrader {
    ...
    event GlobalPaused(uint64 until, address indexed account);
    event GlobalUnpaused(address indexed account);
    event SelectorGlobalPaused(bytes4 indexed selector, uint64 until, address indexed account);
    event SelectorGlobalUnpaused(bytes4 indexed selector, address indexed account);
    ...
    uint64 private globalPausedUntil;
    mapping(bytes4 => uint64) private globalPausedSelectorUntil;
    ...
    function pauseGlobal() external onlyOwner {
        globalPausedUntil = type(uint64).max;
        emit GlobalPaused(globalPausedUntil, msg.sender);
    }

    function unpauseGlobal() external onlyOwner {
        delete globalPausedUntil;
        emit GlobalUnpaused(msg.sender);
    }

    function pauseSelectorGlobal(bytes4 selector) external onlyOwner {
        globalPausedSelectorUntil[selector] = type(uint64).max;
        emit SelectorGlobalPaused(selector, type(uint64).max, msg.sender);
    }

    function unpauseSelectorGlobal(bytes4 selector) external onlyOwner {
        delete globalPausedSelectorUntil[selector];
        emit SelectorGlobalUnpaused(selector, msg.sender);
    }
    ...
    function isPaused(address target) public view returns (bool) {
        return _isActive(globalPausedUntil) || _isActive(pausedUntilByTarget[target]);
    }

    function isPaused(address target, bytes4 selector) public view returns (bool) {
        if (_isActive(globalPausedUntil) || _isActive(pausedUntilByTarget[target])) {
            return true;
        }
        return
            _isActive(globalPausedSelectorUntil[selector]) || _isActive(pausedSelectorUntilByTarget[target][selector]);
    }

    function pausedUntil(address target) external view returns (uint256) {
        uint64 globalUntil = globalPausedUntil;
        uint64 targetUntil = pausedUntilByTarget[target];
        return globalUntil > targetUntil ? globalUntil : targetUntil;
    }

    function pausedSelectorUntil(address target, bytes4 selector) external view returns (uint256) {
        uint64 globalSelectorUntil = globalPausedSelectorUntil[selector];
        uint64 targetSelectorUntil = pausedSelectorUntilByTarget[target][selector];
        return globalSelectorUntil > targetSelectorUntil ? globalSelectorUntil : targetSelectorUntil;
    }
