...
export type ContractAbi<TContractName extends ContractName = ContractName> = typeof ContractABIs[TContractName];
...
type EventArgTypes<TAbiEvent extends AbiEvent> = {
  [Input in TAbiEvent["inputs"][number] as Input extends AbiParameter
    ...
    : unknown]: Input extends AbiParameter
    ? AbiParameterToPrimitiveType<Input>
    : unknown;
};
...
export const getEventFromReceipt = <TContractName extends keyof typeof ContractABIs, TEventName extends ExtractAbiEventNames<ContractAbi<TContractName>>>
  ...
  if (!event) {
    throw new Error(`Event not found in receipt with topic hash: ${hash} and abi: ${JSON.stringify(eventAbi)}`);
  }
