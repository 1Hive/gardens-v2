...
const toastButtons = [
  {
    ...
    handler: () => {
      toast.loading(React.createElement(LoadingToast, { message: "Pulling new data" }), {
...
export default function DesignSystemPage() {
  ...
  const tableColumns = useMemo(
    () => [
      { header: "Name", render: (item: typeof sampleTableData[number]) => item.name },
      { header: "Role", render: (item) => item.role },
      {
        header: "Stake",
        render: (item) => `${item.stake} HNY`,
          ...
          <DemoCard title="Sliders">
            <div className="space-y-4">
              <div>
                ...
                />
                <p className="text-xs text-neutral-soft-content">{sliderValue}% allocated to proposal</p>
              </div>
              <div>
                <label className="block text-sm font-medium">Staking slider</label>
                <input
                  type="range"
                  ...
                  min={0}
                  max={100}
                  onChange={(event) => setStakingSlider(Number(event.target.value))}
                />
                <p className="text-xs text-neutral-soft-content">{stakingSlider}% of balance</p>
              </div>
            </div>
          </DemoCard>
              ...
              <Skeleton isLoading={!showContent} rows={3}>
                <div className="space-y-2">
                  ...
                  <p className="text-neutral-soft-content text-sm">
                    Toggle skeleton to preview transitions between loading and finished states.
                  </p>
                </div>
              </Skeleton>
              <Button size="sm" onClick={() => setShowContent((value) => !value)}>
