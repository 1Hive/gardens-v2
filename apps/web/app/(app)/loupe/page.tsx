...
import IDiamondLoupe from "#/contracts/abis/IDiamondLoupe.sol/IDiamondLoupe.json";
...
const diamondLoupeAbi = IDiamondLoupe.abi;
...
export default function DiamondAdminPage() {
  ...
  const queryDiamondParam =
    queryParams?.diamond ??
    queryParams?.contract ??
    queryParams?.address ??
    queryParams?.target;
  const queryChainParam =
    queryParams?.chainId ?? queryParams?.chain ?? queryParams?.network;

  useEffect(() => {
    const rawDiamond = typeof queryDiamondParam === "string" ?
      queryDiamondParam.trim()
    : undefined;
    if (!rawDiamond || !isAddress(rawDiamond)) {
      return;
    }
    const normalized = rawDiamond as Address;
    setAddressInput((prev) =>
      prev?.toLowerCase() === normalized.toLowerCase() ? prev : normalized,
    );
    setDiamondAddress((prev) =>
      prev?.toLowerCase() === normalized.toLowerCase() ? prev : normalized,
    );
  }, [queryDiamondParam]);

  useEffect(() => {
    const rawChain = typeof queryChainParam === "string" ?
      queryChainParam.trim()
    : undefined;
    if (!rawChain) {
      return;
    }

    const numericChain = Number(rawChain);
    if (!Number.isNaN(numericChain)) {
      const chainExists = CHAINS.some((chain) => chain.id === numericChain);
      if (chainExists) {
        setSelectedChainId((prev) =>
          prev === numericChain ? prev : numericChain,
        );
      }
      return;
    }
    ...
    const matchedChain = CHAINS.find((chain) => {
      const slug = chain.network ?? chain.name;
      return slug?.toLowerCase() === rawChain.toLowerCase();
    });
    if (matchedChain) {
      setSelectedChainId((prev) =>
        prev === matchedChain.id ? prev : matchedChain.id,
      );
    }
  }, [queryChainParam]);

  const normalizedInput = addressInput.trim();
  const isAddressValid = Boolean(normalizedInput) && isAddress(normalizedInput);
  ...
  const handleInspectClick = () => {
    ...
    const normalized = normalizedInput as Address;
    if (diamondAddress === normalized) {
      refetch?.();
      return;
    }
    if (normalized !== addressInput) {
